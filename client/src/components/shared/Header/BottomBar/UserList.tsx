...
const UserList = ({ size = 'md', className }: { size?: 'sm' | 'md' | 'lg'; className?: string }) => {
        ...
        >
          <div className='relative overflow-hidden rounded-full bg-white'>
            ...
            <img
              src={item.image}
              alt={item.name}
              className={cn('rounded-full object-cover', sizes[size] ?? sizes.sm)}
            />
          </div>
          <div className='absolute bottom-full left-1/2 -translate-x-1/2 translate-y-2 transform whitespace-nowrap rounded bg-white p-2  opacity-0 transition-all duration-300 ease-in-out group-hover:-translate-y-2 group-hover:opacity-100 dark:bg-slate-100 dark:text-slate-900'>
            <div className='text-xs font-semibold'>{item.name}</div>
            <div className='text-[10px]'>{item.position}</div>
          </div>
        </div>
      ))}
    </div>
